...
## 2024-08-09 - Defer dynamic allocation on network hot paths
**Learning:** Codebase C/ESP8266 Performance Pattern: On high-frequency network hot paths (like `bridge_input_sta`), eagerly performing dynamic memory allocation (`pbuf_alloc`) and buffer copying (`pbuf_copy`) before early return conditions (e.g., local MAC address checks) causes unnecessary heap fragmentation and wastes expensive CPU cycles for packets that end up being dropped.
**Action:** Defer `pbuf_alloc` and `pbuf_copy` until after evaluating all O(1) early return conditions (like MAC address comparisons) to avoid expensive heap operations for non-bridged packets.

## 2024-05-20 - Defer Memory Allocation on Bridge Hotpaths
**Learning:** C/ESP8266 Performance Pattern: On high-frequency network hot paths (like bridge_input_ap and bridge_input_sta), defer dynamic memory allocation (pbuf_alloc) and buffer copying (pbuf_copy) until after evaluating early return conditions (e.g., local MAC address matches). This avoids expensive heap operations for dropped or non-bridged packets.
**Action:** Always check if early returns can be evaluated using existing memory structures before dynamically allocating memory for processing in C network stacks.

## 2024-05-21 - Early Exit Before Expensive Memory Allocations on Hot Paths
**Learning:** C/ESP8266 Performance Pattern: On high-frequency network hot paths (like `bridge_input_sta`), evaluating drop conditions (like `os_memcmp` for loopback MAC addresses) *after* allocating buffers (`pbuf_alloc`) and copying memory (`pbuf_copy`) wastes significant heap operations for packets that are immediately discarded.
**Action:** Always evaluate O(1) conditional drop checks (like pointer array comparisons or boolean flags) *before* O(N) operations like memory allocation and copying to optimize the per-packet processing hot path.
...
## 2024-05-23 - Firewall ACL Packet Inspection
**Learning:** In the `esp_wifi_repeater` codebase, the `acl_check_packet` function is a critical hot path evaluated for every single packet traversing the firewall. The original implementation used a `for` loop with index arithmetic (`&acl[acl_no][i]`) and redundant struct field traversals (`ip_h->src.addr`) on every iteration. Given the ESP8266's limited CPU resources, hoisting invariant memory reads (like `src_addr`) and replacing array indexing with pointer iteration (`my_entry++` to `end`) yields a measurable ~20%+ execution speedup on worst-case rule evaluation (measured via standalone benchmark), allowing the device to process more packets per second without dropping frames.
**Action:** Always identify and hoist invariant network header reads out of iterative matching loops, and prefer pointer boundaries and arithmetic over array indices when scanning contiguous arrays in packet processing paths on embedded targets.
## 2024-05-24 - O(1) CIDR Length Calculation
**Learning:** Calculating a CIDR network prefix length from a subnet mask by using a `for` loop and shifting bits sequentially can take up to 32 iterations. For valid netmasks (contiguous 1s), this is equivalent to counting the number of set bits.
**Action:** Replace the loop with `__builtin_popcount(mask)`. This built-in function compiles to optimized assembly, calculating the length instantly in O(1) time and saving CPU cycles in tight execution paths.
