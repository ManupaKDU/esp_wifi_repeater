...
## 2024-05-24 - Pre-calculate string lengths for arrays of string literals
**Learning:** Calling `os_strlen` on string literals inside a loop introduces O(N) runtime overhead that can be resolved at compile-time.
**Action:** When iterating over arrays of string literals, pre-calculate their lengths at compile-time into a parallel array (e.g., `const uint8_t txt_len[] = {sizeof("...") - 1, ...}`) to convert O(N) runtime `os_strlen` length evaluations inside loops into O(1) array lookups.
