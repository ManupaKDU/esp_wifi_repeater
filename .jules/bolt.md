...
## 2024-04-04 - Optimize Array String Literal Lengths
**Learning:** Codebase performance pattern: When iterating over arrays of string literals, dynamically calculating lengths with `os_strlen(txt[i])` introduces unnecessary O(N) runtime overhead on every iteration.
**Action:** Pre-calculate their lengths at compile-time into a parallel array (e.g., `const uint8_t txt_len[] = {sizeof("...") - 1, ...}`) to convert O(N) runtime `os_strlen` length evaluations inside loops into O(1) array lookups.
