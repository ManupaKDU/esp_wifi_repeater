...
## 2026-03-14 - Optimize os_strlen in loops with string literals
**Learning:** Codebase performance pattern: Avoid optimizing `os_strlen` inside loops by creating parallel arrays for string literal lengths (e.g., `txt_len` alongside `txt`). This introduces a maintainability flaw due to string duplication. Never apply textbook micro-optimizations (like saving CPU cycles on small strings) to infrequent or I/O-bound paths (like console command handlers) where the performance impact is unmeasurable.
**Action:** When evaluating literal string length optimization on non-critical paths, prefer readability over micro-optimizations. If a compile-time optimization is necessary for a critical loop over strings, prefer a structured pairing pattern (e.g., `struct { const char *str; size_t len; } txt[]`) to eliminate duplication risks.  
