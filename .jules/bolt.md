...
## $(date +%Y-%m-%d) - Optimize dhcp_find_option loop with pointer arithmetic
**Learning:** In performance-critical network parsing loops (like DHCP option finding in ESP8266 code), replacing array indexing (e.g., `opts[i]`) and index-based loop conditions (`i < len`) with pointer arithmetic (`*p`) and a pre-calculated end pointer (`end = opts + opts_len`, `p < end`) eliminates per-iteration index arithmetic and bounds recalculations. This yields more efficient machine code on size-optimized embedded targets.
**Action:** When working on tight C loops for embedded systems, actively look for opportunities to replace array indexing with pointer arithmetic and pre-calculate invariant boundary markers before entering the loop.
