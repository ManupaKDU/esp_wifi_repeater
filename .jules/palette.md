...
## 2026-08-25 - [Disabled State Specificity Fix for Labels]
**Learning:** When styling disabled checkboxes that are wrapped inside a `<label>`, applying `:disabled` to the input correctly disables the input itself, but leaves the wrapping text (like "Show password") with the default pointer cursor.
**Action:** Use `label:has(input:disabled) { cursor: not-allowed; opacity: 0.6; }` to propagate the disabled styling up to the label, providing a correct and unified visual affordance.
