...
## 2025-01-28 - Cross-Variant Accessibility Parity
**Learning:** In C/C++ embedded codebases that heavily use preprocessor directives (like `#ifndef REPEATER_MODE`) to serve different HTML configuration pages based on build variant, UX and accessibility features are often inconsistently applied or entirely missed in secondary compilation blocks. Implementing `for`/`id` linking, ARIA labels, or inline JavaScript logic in the primary block without simultaneously porting those exact structural changes to the secondary `#else` block leads to fragmented UX and a brittle, inaccessible product across deployment modes.
**Action:** Always use tools like `grep` to trace HTML structures through all compilation variants in header files. When making UX enhancements, ensure absolute parity across all conditionally compiled views.
