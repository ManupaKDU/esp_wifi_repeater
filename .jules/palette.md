...
## 2026-06-03 - Explicitly verify cross-variant UX parity in C macros
**Learning:** When upgrading accessibility or UX elements (like adding semantic `<label>`s and conditional disabling) in C macro HTML blocks with multiple compilation variants (e.g.,  and ), it is dangerous to assume all variants have identically structured starting HTML. The NAT mode and Repeater mode variants might have subtly different form structures or missing elements.
**Action:** Always explicitly extract and visually verify the exact HTML structure of *every* target compilation variant block before applying parity updates, to avoid attempting to replace non-existent elements or breaking the layout.
