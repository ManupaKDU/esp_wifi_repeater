...
## 2025-01-28 - Missing Playwright system dependencies in Docker
**Learning:** When using Playwright for local frontend verification inside a clean Docker container, installing `@playwright/test` via `npm/pnpm` is not enough. The `chromium` binary requires system-level libraries (like `libx11`, `libgbm`, etc.) to run, even in headless mode, which will cause immediate failures upon `chromium.launch()`.
**Action:** When writing Playwright setup steps, always include `--with-deps chromium` during the `playwright install` phase (e.g., `pnpm exec playwright install --with-deps chromium`) to automatically fetch required system packages and prevent pipeline failures.
