...
## 2024-06-25 - Robust Focus Indicators
...
**Learning:** Removing native outlines (`outline: none`) on custom-styled elements without providing a robust alternative (like `box-shadow` or `outline`) harms keyboard accessibility.
**Action:** Always provide strong `:focus` or `:focus-visible` indicators when overriding native outlines.
## 2024-07-02 - [Accessibility] Robust focus indicators for custom inputs
**Learning:** Removing native outlines (`outline: none`) on custom-styled elements without providing a robust alternative (like `box-shadow` or `outline`) harms keyboard accessibility. Simply changing the `border-color` is often insufficient for users relying on keyboard navigation to see focus state clearly.
**Action:** Always provide strong `:focus` or `:focus-visible` indicators (such as `box-shadow` or `outline` with offsets) when overriding native outlines to ensure interactive elements are clearly visible when focused via keyboard.
## 2026-06-25 - [Accessibility] Robust Focus Indicators
**Learning:** Removing native outlines (`outline: none`) on custom-styled elements without providing a robust alternative (like `box-shadow` or custom `outline`) harms keyboard accessibility.
**Action:** Always provide strong `:focus` or `:focus-visible` indicators when overriding native outlines.
## 2026-06-25 - Robust Focus Indicators for Custom Outlines
**Learning:** Removing native outlines (`outline: none`) on custom-styled elements without providing a robust alternative harms keyboard accessibility. Relying solely on `border-color` changes is often insufficient to indicate focus.
**Action:** Always provide strong `:focus` or `:focus-visible` indicators (such as `box-shadow` or `outline`) when overriding native outlines to ensure clear keyboard accessibility.
...
**Learning:** In constrained embedded HTML interfaces that rely on generic form submission reloads (e.g., redirecting to the same page with query parameters), using a single static "Settings sent" success message for all forms (like Restarting, Locking, or Saving Settings) causes significant user confusion and lack of confidence in destructive actions.
**Action:** When working with single-page embedded config forms, parse client-side query parameters (e.g., `window.location.search`) to display dynamic, context-aware feedback corresponding to the specific form action the user just took (e.g., "The device is restarting...").
## 2026-06-25 - Context-Aware Feedback in Embedded UIs
**Learning:** In embedded HTML interfaces without full page reloads, using a generic submission success message for all actions (like "Settings sent") causes user confusion when performing distinct, non-configuration actions such as restarting the device or locking it.
**Action:** Always parse client-side query parameters (e.g., `window.location.search`) to dynamically display context-aware feedback (e.g., "The device is restarting...") to give users clear and accurate confirmation of their specific action.
## 2024-05-24 - Dynamic Context-Aware Submission Feedback
**Learning:** In embedded HTML interfaces that lack full page reloads, using a generic submission success message for all actions (e.g., 'Settings sent') causes user confusion for distinct actions like device restarts.
**Action:** Parse client-side query parameters (e.g., `window.location.search`) to dynamically display context-aware feedback (e.g., 'The device is restarting...') when handling distinct form submissions that target the same single-page endpoint.
## 2026-07-18 - Context-Aware Submission Feedback
**Learning:** In embedded HTML interfaces that lack full page reloads, using a generic submission success message for all actions (e.g., 'Settings sent') causes user confusion for distinct actions like device restarts.
**Action:** Parse client-side query parameters (e.g., `window.location.search`) to dynamically display context-aware feedback (e.g., 'The device is restarting...') for a smoother, more informative user experience.
## 2026-07-19 - [Context-Aware Feedback]
**Learning:** In embedded HTML interfaces without full page reloads, using a generic submission success message ("Settings sent") causes user confusion for distinct actions like device restarts.
**Action:** Parse client-side query parameters (`window.location.search`) to dynamically display context-aware feedback (e.g., 'The device is restarting...') for better clarity.
