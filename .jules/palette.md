...
## 2024-05-18 - Replacing destructive DOM updates
**Learning:** For form submission feedback in embedded web interfaces, replacing `document.body.innerHTML` destructively breaks layout and screen reader context.
**Action:** Toggle the visibility of a pre-rendered `<div role='status'>` and update `document.title` to robustly announce state changes without layout shifts.
