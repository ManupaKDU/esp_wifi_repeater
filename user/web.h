...
<style>:focus-visible { outline: 2px solid currentColor; outline-offset: 2px; } :disabled { cursor: not-allowed !important; opacity: 0.6; } label:has(input:disabled) { cursor: not-allowed !important; opacity: 0.6; } input[type='submit'], input[type='checkbox'], label, select { cursor: pointer; } input[type='submit']:hover:not(:disabled), select:hover:not(:disabled) { opacity: 0.8; transition: opacity 0.2s; }</style>\
