...
<div id='status_msg' role='status' style='display:none;'>The new settings have been sent to the device...</div>\
<div id='config'>\
<script>\
...
document.title = 'Settings Sent - ESP WiFi NAT Router';\
...
<body>\
<h1>ESP WiFi NAT Router Config</h1>\
<div id='status_msg' role='status' style='display:none;'>Unlock request has been sent to the device...</div>\
<div id='config'>\
<script>\
if (window.location.search.substr(1) != '')\
{\
document.getElementById('config').style.display = 'none';\
document.getElementById('status_msg').style.display = 'block';\
document.title = 'Unlock Sent - ESP WiFi NAT Router';\
