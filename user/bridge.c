...
static err_t ICACHE_FLASH_ATTR bridge_input_ap(struct pbuf *p, struct netif *inp)
{
    ...
    /* ⚡ Bolt: Defer pbuf_alloc on the AP hot path until we are sure we are not dropping
       the packet. This avoids expensive heap allocation for early dropped packets. */
...
static err_t ICACHE_FLASH_ATTR bridge_input_sta(struct pbuf *p, struct netif *inp)
{
    ...
    /* ⚡ Bolt: Defer pbuf_alloc on the STA input hot path until we are sure we are not
       dropping the packet. */
    eth_hdr_t *eth_p = (eth_hdr_t *)p->payload;
