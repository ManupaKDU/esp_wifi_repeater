...
void ICACHE_FLASH_ATTR timer_func(void *arg)
{
    ...
    if (mqtt_enabled && config.mqtt_interval != 0 && (t_diff > config.mqtt_interval))
    {
        ...
        /* ⚡ Bolt: Securely bypass expensive formatting and API calls when no topics are masked */
            ...
            if (buffer != NULL)
            {
                ...
                while (station)
                {
                    if (do_colon) {
                        ...
                        // ⚡ Bolt: Optimize single character append. Eliminate os_sprintf parsing
                        // overhead by using direct array assignment and manual null-termination.
                        buffer[len + 1] = '\0';
                        len += 1;
                    }
                    do_colon = true;
