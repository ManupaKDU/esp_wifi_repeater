...
// ⚡ Bolt: Provide a length-aware variant to avoid redundant os_strlen() overhead
...
void ICACHE_FLASH_ATTR mqtt_publish_int(uint16_t mask, uint8_t *sub_topic, uint8_t *format, uint32_t val)
{
    ...
    // ⚡ Bolt: Cache redundant os_strlen() call by reusing the length returned by os_sprintf
